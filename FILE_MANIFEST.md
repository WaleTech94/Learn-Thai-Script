...
Verified current app version: `v6.2.0`.
Verified current service-worker cache: `aan-thai-v6-2-0`.
...
- `dist/aan-thai-v6.2.0-source.zip` - generated on demand by `tools/make-release-zip.sh`; ignored and not tracked.
...
The helper writes `dist/aan-thai-v6.2.0-source.zip` using `git archive`, so it includes tracked files from `HEAD` only. Commit or stash tracked edits before packaging if the zip must include them.
