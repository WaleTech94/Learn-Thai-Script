...
Verified current app version: `v8.1.0`.
Verified current footer string: `Phase 1 complete (v8.1.0)`.
Verified current service-worker cache: `aan-thai-v6-4-1` (unchanged in v8.1.0 because cached asset filenames did not change).
...
- `dist/aan-thai-v8.1.0-source.zip` - generated on demand by `tools/make-release-zip.sh`; ignored and not tracked.
...
The helper writes `dist/aan-thai-v8.1.0-source.zip` using `git archive`, so it includes tracked files from `HEAD` only. Commit or stash tracked edits before packaging if the zip must include them.
