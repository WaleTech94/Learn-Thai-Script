...
Verified current app version: `v8.2.1`.
Verified current footer string: `Conversation first + reading (v8.2.1)`.
Verified current service-worker cache: `aan-thai-v8-2-1` (advanced so installed copies refresh the changed cache-first manifest description).
...
- `dist/aan-thai-v8.2.1-source.zip` - generated on demand by `tools/make-release-zip.sh`; ignored and not tracked.
...
The helper writes `dist/aan-thai-v8.2.1-source.zip` using `git archive`, so it includes tracked files from `HEAD` only. Commit or stash tracked edits before packaging if the zip must include them.
