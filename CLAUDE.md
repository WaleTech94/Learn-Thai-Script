...
**Current state:** Phase 1 (script mastery) complete and live at **v3.5**. The letter → class → tone spine and its objective mastery gates remain intact. v3.5 keeps the 45-minute Today plan, decode-first lesson flow, no-answer-audio sound recall, definition-light quiz layer, vowel-carrier clarity, and removes English-cue spoken vocabulary retrieval from Phase 1 lessons. Rewards are quieter and more useful: Library unlocks phrase packs and extras without selling progress; the evidence profile is diagnostic only. Phases 2–3 are not yet built.
...
- **Audio:** browser `speechSynthesis` with the device Thai voice (iOS: Kanya). Speech is primed on first touch (iOS gesture requirement) and resumed on foreground (iOS leaves it paused). Tones from synthetic voices are approximate — the UI says so. Vowel cards label both the target vowel sound and the carrier form because isolated Thai vowels need a visible/speakable carrier. v2.35 also uses `MediaRecorder` for optional, private record/playback comparison; recordings remain local and are discarded when the learner leaves the activity.
...
- **Lesson player flow (v3.1–v3.5):** concept pages (`pre`) → glyph cards → **Quick decode** (`we`: compact class-colour/code reminder + matched "you do" MCQ) → word cards where Thai appears first and transliteration/audio/meaning unlock only after the learner reveals the reading → script-cued **Read & say** with optional local record/playback comparison → objective quiz. Routine definition questions do not appear in lesson quizzes; meaning stays on reveal, while quiz slots focus on sound, class, reading, tone, mechanism checks and heard-word→Thai recognition for `core` words. Sound-recall questions are retrieval-only: no pre-answer audio on "What sound?" prompts. Vowel glyph cards label the target sound separately from the อ carrier-form example.
