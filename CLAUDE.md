...
**Current state:** Phase 1 (script mastery) complete and live at **v3.3**. The letter → class → tone spine and its objective mastery gates remain intact. v3.3 keeps the 45-minute Today plan, decode-first lesson flow, no-answer-audio sound recall, and removes routine definition checks from lesson quizzes so early testing stays focused on script mastery rather than vocabulary meaning. Rewards are quieter and more useful: Library unlocks phrase packs and extras without selling progress; the evidence profile is diagnostic only. Phases 2–3 are not yet built.
