...
**Goal:** beginner → conversational Thai (CEFR A0 → ~B1). Built as a phased long-term project, ~30 min/day budget.
**Current state:** Phase 1 (script mastery) complete and live at **v2.36**. The letter → class → tone spine and its objective mastery gates remain intact. v2.35 integrated sound, script and spoken retrieval; v2.36 then audited all 96 lesson words so incidental decoding examples no longer consume production practice or meaning-test space. Phases 2–3 are not yet built.
...
- **Deployed as a PWA** on GitHub Pages. Companion files: `manifest.json`, `sw.js` (service worker: network-first for the shell so updates propagate, cache-first for assets), `icon-180/192/512.png`.
- **Deploy loop:** replace `index.html` in the repo (Add file → Upload files → commit, same filename overwrites). Pages rebuilds in ~60s. On iPhone: open app online, swipe closed, reopen. Version string is at the bottom of the Home tab.
- **Persistence:** `localStorage` key `thai_state_v1`, with a fallback to Codex's artifact `window.storage` when run inside Codex. Saves are debounced 350ms and **flushed synchronously on `visibilitychange`/`pagehide`** (iOS swipe-away protection). Export/Import buttons (Home, bottom) move state as a JSON blob.
...
- **Lesson player flow (v2.35–v2.36):** hear `core` + `recognition` words attached to meaning before seeing spelling → concept pages (`pre`) → glyph cards → **worked example** (`we`: an "I do" decode chain + matched "you do" twin MCQ) → words page with **guess-then-reveal meanings** and visible RECOGNISE / DECODE ONLY labels → spoken retrieval from an English cue for `core` words only + optional local record/playback comparison → objective quiz. Meaning questions cover `core` + `recognition`; lesson listening questions use `core` only; `decode` words never generate vocabulary questions.
...
- **Daily plan (Home):** day type frozen at first open — **due > 45 (or course complete) → Consolidation day**, else Lesson day with the named next lesson + a rotating drill suggestion. Weekly status line: phrase pack x/5, unread story, licence expiry. **(v2.23: a daily pacing governor surfaces Learn → Consolidate → Rest on the path hero — soft, "keep going anyway" always available — and mastery checkpoints gate new lessons at ≥80%.)**
...
- **Phrases pipeline:** weekly pack of 5 auto-added to SRS (category order Politeness → Numbers → Food → Taxi → Survival → Shopping → owned packs), "used it on a real human" = +5💎 each. Category ✓ when all phrases reach 7+ day intervals.
...
**Earn:** daily plan +10 · spell-it first-tries +2 · class sprint ≥85% +3 · story first read +3 · pack phrase used IRL +5 · mastery check first pass +15 · tone licence +30/+10 · new personal-best flashcard lap +3. (Wager ±stake earnings removed in v2.21; per-unit boss +20 retired v2.23 in favour of mastery checkpoints.) ~100💎/week when consistent. Earnings board: tap the 💎 chip.
**Spend:** streak freeze 30 (auto-consumed on a single missed day) · slang pack 50 · Teacher/Food+ packs 60 each · themes 40 (Day market light / Skytrain / Songkran) · titles 25 (5 available) · gachapon pulls 10 (24-item "Bangkok streets" set, each capsule = a real word + fact; dupes refund 3).
...
`streak{count,last}` · `done[]` lesson ids · `srs{id:{iv,due,lapses}}` · `baht` · `freeze` · `days{date:{r,l,d,due,paid,type}}` · `calib{1|2|3:{n,c}}` (dormant since v2.21) · `boss{unit:{passed,lastAttempt}}` (dormant since v2.23) · `checks{checkpointId:true}` (v2.23 mastery gate) · `skillStats{listen[],speak:{attempts,last},foundationEar}` (v2.35 separate evidence) · `packs[]` · `stories{id:date}` · `confs{setKey:misses}` · `toneStats{listen[],rule[]}` · `toneLicence{earned,expires}` · `week{key,picks[],used{}}` · `gacha{id:count}` · `themes[]` · `theme` · `titles[]` · `title`.
...
- **Candidates parked:** adaptive difficulty (needs usage data), bet-on-yourself commitment stakes (declined for now), push reminders (**not feasible** — Web Push needs a server; Pages is static).
...
**Version discipline:** bump the version string (bottom of Home tab) on every shipped change and tell the owner the expected version string so they can confirm the deploy landed.
...
git push
```
GitHub Pages rebuilds in ~60s. The owner then opens the PWA on his iPhone 16 (Chrome, installed to home screen) **while online**, swipes it closed, and reopens — the service worker fetches the new shell network-first, so the second launch is fresh. Confirm the version string with the owner.

No web-upload step anymore (that was the pre-Codex workflow). Keep `manifest.json`, `sw.js`, and icons in the repo root; bump the `CACHE` constant in `sw.js` only if cached asset filenames change.
