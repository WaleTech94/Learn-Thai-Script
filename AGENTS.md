...
**Current state:** Phase 1 (script mastery) complete and live at **v3.0**. The letter → class → tone spine and its objective mastery gates remain intact. v3.0 reframes the app around a 45-minute Today plan: clear current review, complete one main lesson/consolidation block, then deepen current material instead of rushing ahead. Rewards are quieter and more useful: Library unlocks phrase packs and extras without selling progress; the evidence profile is diagnostic only. Phases 2–3 are not yet built.
...
- **Deploy loop:** replace `index.html` in the repo (Add file → Upload files → commit, same filename overwrites). Pages rebuilds in ~60s. On iPhone: open app online, swipe closed, reopen. Version string is at the bottom of the Today tab.
