...
**Current state:** Phase 1 (script mastery) complete and live at **v3.1**. The letter → class → tone spine and its objective mastery gates remain intact. v3.1 keeps the 45-minute Today plan from v3.0 but corrects the first-use learning sequence: the path opens with class colour/code before Lesson 1, lesson words are decoded before transliteration/audio/meaning are revealed, and the old bloated "I do" worked-example label is now a lean Quick decode check. Rewards are quieter and more useful: Library unlocks phrase packs and extras without selling progress; the evidence profile is diagnostic only. Phases 2–3 are not yet built.
