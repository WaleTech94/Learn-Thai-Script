...
**Current state:** Phase 1 (script mastery) complete and live at **v3.4**. The letter → class → tone spine and its objective mastery gates remain intact. v3.4 keeps the 45-minute Today plan, decode-first lesson flow, no-answer-audio sound recall, definition-light quiz layer, and clarifies vowel cards by separating the target vowel sound from the อ carrier-form example. Rewards are quieter and more useful: Library unlocks phrase packs and extras without selling progress; the evidence profile is diagnostic only. Phases 2–3 are not yet built.
