...
function renderMarkdown(audit){
  ...
  lines.push('');
  lines.push("v8.1.0 is the Street Arcade retention-and-fun pass: Parcel Sort unlocks free after Lesson 2, Night Market Hunt costs 50 tokens after Lesson 4, and Tuk-Tuk Tone Run costs 60 tokens after Lesson 13; every cabinet rebuilds from completed lessons through the existing glyph, mechanism and prerequisite checks, never serves FRESH_DECODE, ASSESSMENT_BANK or RETENTION_DECODE_BANK words, weights selected rounds by existing weakness diagnostics, stores only optional personal records in drillLog, and never changes SRS, blockers, mastery, required Today work, streaks or repeatable token earnings. Three paid themes (Ekkamai Sunset, Yaowarat Neon and Tuk-Tuk Chrome) add code-native motifs and temporary shop previews; all arcade feedback, engine, market, sorting, combo and completion cues are synthesized locally through the existing sound toggle and selected sound voice. validateV81ArcadeContracts brings the generated audit to 59 validators.");
  lines.push('');
