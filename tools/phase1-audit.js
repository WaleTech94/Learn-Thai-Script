...
globalThis.__phase1Audit = (function(){
  ...
  const validators = [
    ...
    isolatedValidatorResult('conversationLearning', validateConversationContracts)
  ];
  return {
...
function renderMarkdown(audit){
  ...
  lines.push('');
  lines.push("v8.5.0 adds the What stuck? diagnostic with separate listening, reply-choice, reported-recall and prerequisite-safe reading results. Week 1 uses shared choice/recall views, cue-derived answers, successful playback gates, explicit recall and complete repair exchanges; lesson estimates are 12/14/14 minutes. Reading has a prominent Today entry. Authored data, course authority and supplementary checks are separate local scripts. Active Lesson 3 +7 forms use Week 1 contexts; archived forms preserve history. Schema-2 progression, Phase 1 authority and device-TTS scope remain intact. validateConversationContracts keeps the audit at 60 validators.");
  lines.push('');
