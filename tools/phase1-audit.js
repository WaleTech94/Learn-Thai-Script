...
function renderMarkdown(audit){
  ...
  lines.push('');
  lines.push("v8.2.1 corrects the product front door: first-run onboarding, masthead, Today hierarchy, About copy and manifest now state usable conversational Thai for daily Bangkok life as the primary goal; finishing onboarding launches the food-ordering scene; the spoken card appears first as the clear recommendation; and the complete Phase 1 route is labelled an optional reading companion that never gates speaking. validateV821ConversationFrontDoorContracts brings the generated audit to 61 validators. The v8.2.0 scene and isolated conversation state remain unchanged: one 8-12 minute gist/listen/respond/substitute/role-play loop, no SRS, mastery, blocker, required-route, streak, token or active-time mutation, device voice labelled rough preview and authored Thai still teacher-review pending.");
  lines.push('');
