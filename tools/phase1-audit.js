...
globalThis.__phase1Audit = (function(){
  ...
  const validators = [
    ...
    isolatedValidatorResult('v830ConversationCourse', validateV830ConversationCourseContracts)
  ];
  return {
...
function renderMarkdown(audit){
  ...
  lines.push('');
  lines.push("v8.3.0 replaces the one-scene pilot with the first complete Bangkok Conversation Foundation unit: three meaning-first lessons, an eight-objective consolidation, three disjoint 12-objective gate forms, exact +1/+7/+30 retention forms, a two-check backlog governor and Bangkok-calendar scheduling. Lesson, cold-check and spoken-participation evidence remain separate; first attempts survive repair; action-boundary resume never stores audio; schema-1 history migrates without authority; strict schema-2 import and local quarantine protect progress. Device Thai TTS remains the only pronunciation model, no native recording or reviewer is assumed, and Phase 1 reading state remains unchanged behind its optional companion controls. validateV830ConversationCourseContracts brings the generated audit to 60 validators.");
  lines.push('');
