...
globalThis.__phase1Audit = (function(){
  ...
  const validators = [
    ...
    isolatedValidatorResult('v81Arcade', validateV81ArcadeContracts),
    isolatedValidatorResult('v82ConversationPilot', validateV82ConversationPilotContracts)
  ];
  return {
...
function renderMarkdown(audit){
  ...
  lines.push('');
  lines.push("v8.2.0 is the isolated conversation-first field test: one optional 8-12 minute Bangkok food-ordering scene moves through gist listening, six male-polite learner chunks, vendor-response selection, temporary local record/playback, one substitution and supported role-play. It stores only bounded run count, first/last completion day and latest self-rating under optional conversation state; it does not touch lessons, SRS, mastery, blockers, required Today work, streaks, tokens, active-time credit, error profiles, drill records or phrase review. Device voice is labelled a rough preview and the authored Thai is teacher-review pending. validateV82ConversationPilotContracts brings the generated audit to 60 validators. v8.1.0 remains the frozen Street Arcade/Phase 1 baseline: Parcel Sort unlocks free after Lesson 2, Night Market Hunt costs 50 tokens after Lesson 4, and Tuk-Tuk Tone Run costs 60 tokens after Lesson 13; every cabinet rebuilds from completed lessons through existing glyph, mechanism and prerequisite checks, sealed corpora never enter play, and no SRS, blocker, mastery, required-route, streak or repeatable-token rule changes.");
  lines.push('');
