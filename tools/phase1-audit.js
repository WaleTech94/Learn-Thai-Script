...
globalThis.__phase1Audit = (function(){
  ...
  const validators = [
    ...
    isolatedValidatorResult('v821ConversationFrontDoor', validateV821ConversationFrontDoorContracts),
    isolatedValidatorResult('v822BeginnerConversation', validateV822BeginnerConversationContracts),
    isolatedValidatorResult('v823ConversationPractice', validateV823ConversationPracticeContracts)
  ];
  return {
...
function renderMarkdown(audit){
  ...
  lines.push('');
  lines.push("v8.2.3 makes the first conversation lesson completable only through its actual supported practice: six segmented cue/reply pairs must play, the full scene highlights each active turn above a collapsed transcript, the four active replies use rotated choices with wrong-answer repair and retry, recording is one optional phrase, the food substitution is pre-taught, and four supported role-play models are checked before the final rating. The resulting current-day spoken-minimum status is derived from bounded interaction evidence inside conversation state; the complete Phase 1 literacy surface remains collapsed and optional, with SRS, lessons, rewards, streak and active-time state unchanged. Device TTS remains the only audio model. validateV823ConversationPracticeContracts brings the generated audit to 63 validators.");
  lines.push('');
