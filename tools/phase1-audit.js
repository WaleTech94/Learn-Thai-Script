...
globalThis.__phase1Audit = (function(){
  ...
  const validators = [
    ...
    isolatedValidatorResult('v840ConversationBuilder', validateV840ConversationBuilderContracts)
  ];
  return {
...
function renderMarkdown(audit){
  ...
  lines.push('');
  lines.push("v8.4.0 rebuilds the Week 1 learner experience around short listen-build-speak loops. Every new reply is assembled from tappable phrase parts with meaning and pronunciation help, rebuilt with less support, heard in role and then changed once for transfer. Vendor and learner lines use different installed Thai voices when two are available; a one-voice device gets small rate separation and never artificial pitch changes. Full conversations use explicit pauses, including a longer meal-to-payment pause. Lesson totals are now 10, 12 and 12 minutes; consolidation, gate and delayed reviews are 10, 12 and 3/5/8 minutes. The schema-2 authority, exact gate forms, delayed evidence, resume, recovery, male-polite Thai and optional reading boundary are preserved. validateV840ConversationBuilderContracts keeps the generated audit at 60 validators.");
  lines.push('');
