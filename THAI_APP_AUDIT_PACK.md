...
This v5.4.2 pass keeps the v5.4.1 automatic weak-correct timing, then strips verbose scheduler language from learner-facing Today and Course Map review copy. Task cards now use short status such as `Clear`, `29 due`, `Review first`, `Done` and `Practice` instead of explaining hard caps, default slices or catch-up policy. It keeps Phase 1 as a script/decoding trainer, with no Phase 2, backend, human-audio pipeline, AI audio, scraped audio or broad phrasebook module.
...
- Service worker Cache API cache name in `sw.js`: `aan-thai-v5-4-2`.
...
Feedback: toasts, result screens, targeted wrong-answer explanations, class colours, capability lines, Daily win card, personal best lap, weak-correct/leech statuses and token awards.
...
`sw.js`: service worker. Network-first for shell and cache-first for other assets. Cache name now matches the current v5.4.2 release family: `aan-thai-v5-4-2`.
...
`AGENTS.md`: canonical project instructions and architecture/pedagogy context. Its top "Current state" now matches the live app version, `v5.4.2`.

`CLAUDE.md`: local mirror of project instructions. Its top "Current state" now matches `AGENTS.md` and the live app version, `v5.4.2`.

`CHANGELOG.md`: release history. Current top entry is `v5.4.2 - 2026-06-29`, describing streamlined Today review language and the new copy validator.

`docs/phase1_audit.md`: generated readable Phase 1 audit. Refreshed during this pack; reports app version `v5.4.2`, 24 lessons, six fluency reads and all validators passing.
...
`release-review-packet/`: old v5.1 senior-developer review handoff. Useful historical context, but stale for current v5.4.2.
...
```text
node -e "const fs=require('fs'); const html=fs.readFileSync('index.html','utf8'); const match=html.match(/<script>([\s\S]*)<\/script>\s*<\/body>/); if(!match){throw new Error('embedded script not found')} fs.writeFileSync('/tmp/thai-app-script.js', match[1]); console.log('wrote /tmp/thai-app-script.js');"
exit code 0
stdout: wrote /tmp/thai-app-script.js
```
...
Version sync: the live footer/generated audit/changelog, `AGENTS.md` and `CLAUDE.md` all describe the current app as `v5.4.2`. The generated audit workload narrative still describes the v5.4 fluency-read scope where relevant.
...
Service worker cache name: `sw.js` now uses `aan-thai-v5-4-2`, aligned with the current release family. The existing network-first shell strategy is unchanged.
...
Audio reliability: Thai voice availability is device/browser-dependent. The app has setup guidance, but browser TTS is device voice support only. There are no human-audio assets or fallback hooks in v5.4.2.
...
- Are intervals, hidden weak-correct handling, leech handling and daily caps pedagogically sensible?
